...
export default function TeamSignup() {
    ...
    const onSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            // Log non-OK responses and non-JSON content-types for easier debugging
            const ct = res.headers.get('content-type') || '';
            if (!res.ok) {
                const text = await res.text(); // this will capture HTML error pages, too
            ...
            setMsg(`Success: team ${data.teamId}`);
                ...
                <CardHeader className="text-center pb-8">
                    ...
                    <div className="flex justify-center items-center gap-4 mt-4 text-sm">
                        ...
                        <div className="bg-cyan-500/20 px-3 py-1 rounded-full border border-cyan-400/30">
                            <span className="text-cyan-300">Team Size: 3 Members</span>
                        </div>
                    </div>
                </CardHeader>
                ...
                <CardContent className="space-y-6">
                    <form onSubmit={onSubmit} className="space-y-6">
                        ...
                        <Card className="bg-cyan-950/30 border-cyan-500/20">
                            <CardHeader className="pb-4">
                                <CardTitle className="text-cyan-200 text-lg">Team Members</CardTitle>
                            </CardHeader>
                            <CardContent className="space-y-6">
                                {members.map((m, i) => (
